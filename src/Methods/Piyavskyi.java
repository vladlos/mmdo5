...
public abstract class Piyavskyi extends GlobalMinimum {
    ...
    public double findMin(double leftBound, double rightBound, double L, double epsilon) {
        ...
        do {
            ...
            i = 1;
            while (y.get(i) < z.get(iMin) && i < y.size() - 2) {
                i++;
            }

            for (int k = sizeFz - 1; k >= i; k--) {
                y.set(k + 1, y.get(k));
            }
            ...
            z.set(sizeFz - 1, minorantZ0(y.get(i), y.get(i + 1), L));
            fz.set(sizeFz - 1, minorantZF0(z.get(sizeFz - 1), y.get(i), L));
